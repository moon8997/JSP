...
import javax.servlet.http.HttpServletResponse;
                                                        ...
                                                        //예>Command.properties 파일에 회원관리 게시판관리 등 하나하나 만들기      
@WebServlet(
...
public class FrontController extends HttpServlet {
	...
	Map<String, Object> cmdMap = new HashMap<>();  //이 클래스의 객체이든지 다 받아야함 다향성
	//객체생성->저장->사용   //cmdMap 저장소?만듬
  ...
  */
       ...
       // System.out.println("!!! 확인용 props => " + props);  
		...
		try {                       //ㅂ19줄에 이름인 (에>o:/NCS/w~~이렇게 잘못 쓸경우를 대비해서 
		 ...
		 // Properties 는 Collection 중 HashMap 계열중의  하나로써
		    ...
		    pr.load(fis); 
		    /*
	         pr.load(fis); 은  fis 객체를 사용하여 C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties 파일의 내용(키와 벨류)을 읽어다가 
	      ...
	      */
	       ...
	       */
		    ...
		    while(en.hasMoreElements()) {
		    	...
		    	String key = (String) en.nextElement();    //있으면 키값을 가져옴
			...
			e.printStackTrace();
		} catch(Exception e) {   //나머지는 얘가 다 받는다
			e.printStackTrace();
		}
	...
	//	System.out.println("~~ 확인용 key => " + key);
		...
		AbstractController action =	(AbstractController) cmdMap.get(key);  //   /main.up이런거에 대한 것을 불러온다// 어차피 둘다 부모클래스 똑같으므로 부모클래스인 AbstractController로 받아온다
		...
		if(action == null) {
			...
			try {
				...
				if(!bool) {
					...
					if(viewPage != null) {  //ㅂ단순하게 페이지이동만 있음
