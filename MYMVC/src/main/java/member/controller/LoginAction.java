...
public class LoginAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(!"POST".equalsIgnoreCase(method)){
			...
			String loc = "javascript:history.back()";  //이전페이지
	    ...
	    if( loginuser != null) {
	    	...
	    	if(loginuser.getIdle() == 1) {
		    	...
		    	//원래는 위와 같이 index.up이 아니라 휴면인 계정을 풀어주는 페이지로 잡아주여야 한다.
		    	// 세미일때는 index.up이게 아니라 휴면계정풀어주는 페이지로 가기  후면페이지 만들어야한
	    	...
	    	if( loginuser.isRequirePwdChange() == true ) {
		    	...
		    	String loc = request.getContextPath()+"/index.up";
		    	//원래는 위와 같이 index.up이 아니라 암호를 변경해주는 페이지로 잡아주여야 한다.
		    	// 세미일때는 index.up이게 아니라 휴면계정풀어주는 페이지로 가기  후면페이지 만들어야한   마이페이지에서변경
	    		...
	    		if(goBackURL != null) {
	    			...
	    			super.setViewPage(request.getContextPath()+"/index.up");   //session.setAttribute("loginuser", loginuser);  있는상탱세서 /index.up로 간다.
	    		}
	    	}
	    }
