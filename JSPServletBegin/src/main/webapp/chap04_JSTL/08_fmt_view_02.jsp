...
<body>
	...
	<h2>문자열로 되어진 숫자를 정수로 형변환 하여 누적한 결과값 나타내기(JSTL을 사용한것)</h2>
	...
	<c:if test="${ not empty requestScope.pointArr1 }">
			...
			<c:forEach var="point" items="${requestScope.pointArr1}">
				<li>
					...
					<%--
						...
						integerOnly="true" 은소수점은 절삭해버리고 정수만 취해오는 것이다.
						정수만 취해온 값을 변수 pointInt 에 넣ㅇ줌.
					 --%>
					${pointInt}
				</li>
		...
		pointArr1 을 정수만 취해온 누적의 누적의합계 : ${sum} 
	</c:if>
